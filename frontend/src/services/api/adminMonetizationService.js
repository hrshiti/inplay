...
const adminMonetizationService = {
    async getAllPlans() {
    ...
    async getSubscriptionAnalytics() {
        ...
        const data = await response.json();
        if (!data.success) {
            throw new Error(data.message || 'Failed to fetch subscription analytics');
        }

        return data.data;
    },

    async getPaidContentPerformance() {
        const token = localStorage.getItem('adminToken');
        if (!token) throw new Error('No admin token found');

        const response = await fetch(`${API_URL}/payment/admin/content-performance`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`,
            },
        ...
        const data = await response.json();
        if (!data.success) {
            throw new Error(data.message || 'Failed to fetch content performance');
        }
...
export default adminMonetizationService;
