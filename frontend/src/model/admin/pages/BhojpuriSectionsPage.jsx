...
export default function BhojpuriSectionsPage() {
    ...
    const [bhojpuriVideos, setBhojpuriVideos] = useState([]);
    ...
    const fetchSections = async () => {
        setLoading(true);
        try {
            const data = await bhojpuriSectionService.getAllSections();
            setSections(data);
        } catch (error) {
    ...
    const handleCreateSection = async () => {
        ...
        try {
            await bhojpuriSectionService.createSection(newSection);
    ...
    const handleDeleteSection = async (id) => {
        ...
        try {
            await bhojpuriSectionService.deleteSection(id);
            fetchSections();
        } catch (error) {
    ...
    const handleToggleActive = async (section) => {
        try {
            await bhojpuriSectionService.updateSection(section._id, { isActive: !section.isActive });
            fetchSections();
        } catch (error) {
    ...
    const handleUpdateOrder = async (id, newOrder) => {
        try {
            await bhojpuriSectionService.updateSection(id, { order: newOrder });
        } catch (error) {
    ...
    const handleAddVideoToSection = async (section, video) => {
        ...
        try {
            await bhojpuriSectionService.updateSection(section._id, { videos: updatedVideos });
            fetchSections();
        } catch (error) {
    ...
    const filteredBhojpuriVideos = bhojpuriVideos.filter(qb => 
    ...
    return (
        <div style={{ padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <div>
                    <h1 style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#111827' }}>Bhojpuri Sections Management</h1>
                    <p style={{ color: '#4b5563', fontSize: '0.9rem' }}>Create and organize dynamic InPlay Bhojpuri sections</p>
                </div>
                <button
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px auto auto', gap: '16px', alignItems: 'end' }}>
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. Trending Bhojpuri"
                            />
                        </div>
                        <div>
                            ...
                            <div style={{ padding: '20px', background: '#fff', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                                        ...
                                        {section.videos && section.videos.map((video, idx) => (
                                            <div key={video._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: '#f1f5f9', borderRadius: '8px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                    ...
                                                    {video.poster && <img src={getImageUrl(video.poster.url)} alt={video.title} style={{ width: '40px', height: '40px', objectFit: 'cover', borderRadius: '4px' }} />}
                                ...
                                <div style={{ borderLeft: '1px solid #e2e8f0', paddingLeft: '24px' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px', background: '#f8fafc', padding: '8px 12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                                        ...
                                        <input 
                                            type="text" 
                                            placeholder="Search bhojpuri videos..." 
                                            value={searchQuery}
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '350px', overflowY: 'auto' }}>
                                        {filteredBhojpuriVideos.map(video => {
                                            ...
                                            return (
                                                <div key={video._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: isAdded ? '#f1f5f9' : '#fff', border: '1px solid #e2e8f0', borderRadius: '8px' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                        {video.poster && <img src={getImageUrl(video.poster.url)} alt={video.title} style={{ width: '32px', height: '32px', objectFit: 'cover', borderRadius: '4px' }} />}
