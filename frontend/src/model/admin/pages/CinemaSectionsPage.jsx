...
export default function CinemaSectionsPage() {
    ...
    const [CinemaVideos, setCinemaVideos] = useState([]);
    ...
    const fetchSections = async () => {
        setLoading(true);
        try {
            const data = await CinemaSectionService.getAllSections();
            setSections(data);
        } catch (error) {
    ...
    const handleCreateSection = async () => {
        ...
        try {
            await CinemaSectionService.createSection(newSection);
    ...
    const handleDeleteSection = async (id) => {
        ...
        try {
            await CinemaSectionService.deleteSection(id);
            fetchSections();
        } catch (error) {
    ...
    const handleToggleActive = async (section) => {
        try {
            await CinemaSectionService.updateSection(section._id, { isActive: !section.isActive });
            fetchSections();
        } catch (error) {
    ...
    const handleUpdateOrder = async (id, newOrder) => {
        try {
            await CinemaSectionService.updateSection(id, { order: newOrder });
        } catch (error) {
    ...
    const handleAddVideoToSection = async (section, video) => {
        ...
        try {
            await CinemaSectionService.updateSection(section._id, { videos: updatedVideos });
            fetchSections();
        } catch (error) {
    ...
    const filteredCinemaVideos = CinemaVideos.filter(qb => 
    ...
    return (
        <div style={{ padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <div>
                    <h1 style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#111827' }}>Cinema Sections Management</h1>
                    <p style={{ color: '#4b5563', fontSize: '0.9rem' }}>Create and organize dynamic InPlay Cinema sections</p>
                </div>
                <button
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px auto auto', gap: '16px', alignItems: 'end' }}>
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. Trending Cinema"
                            />
                        </div>
                        <div>
                            ...
                            <div style={{ padding: '20px', background: '#fff', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                                ...
                                <div style={{ borderLeft: '1px solid #e2e8f0', paddingLeft: '24px' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px', background: '#f8fafc', padding: '8px 12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                                        ...
                                        <input 
                                            type="text" 
                                            placeholder="Search Cinema videos..." 
                                            value={searchQuery}
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '350px', overflowY: 'auto' }}>
                                        {filteredCinemaVideos.map(video => {
