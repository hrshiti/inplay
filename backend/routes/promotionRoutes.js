...
// Admin routes
router.post('/', protect, authorize('admin', 'superadmin'), requireFreeDiskSpace, createPromotion);
router.get('/all', protect, authorize('admin', 'superadmin'), getAllPromotions); // For admin panel list
router.put('/:id', protect, authorize('admin', 'superadmin'), requireFreeDiskSpace, updatePromotion);
